...
class TwoFactorSetup(LoginRequiredMixin, FormView):
    ...
    def get_qr_code_data_uri(self):
        svg_data, otpauth_code = generate_totp_config_svg_for_device(self.request, self.device)
        qr_code_url = f"data:image/svg+xml;base64,{force_str(b64encode(svg_data))}"
        return qr_code_url, otpauth_code

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["qr_code_url"], context["otpauth_code"] = self.get_qr_code_data_uri()
        return context
